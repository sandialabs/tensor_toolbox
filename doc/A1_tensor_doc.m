...
% |norm| computes the Frobenius norm of a tensor. This corresponds to the
% Euclidean norm of the vectorized tensor.
T = tensor(randn(2,3,3));
norm(T)
%% Using reshape to rearrange elements in a tensor
% |reshape| reshapes a tensor into a given size array. The total number of
% elements in the tensor cannot change.
...
% A tensor can be symmetrized in a collection of modes with the command
% |symmetrize|. The new, symmetric tensor is formed by averaging over all
% elements in the tensor which are required to be equal. 
W = tensor(rand(4,4,4));
Y=symmetrize(X);
%%
% A second argument can also be passed to |symmetrize| which specifies an
% array of modes with respect to which the tensor should be symmetrized.
X = tensor(rand(3,2,3));
Z = symmetrize(X,[1,3]);
%%
% Additionally, one can check for symmetry in tensors with the
% |issymmetric| function. Similar to |symmetrize|, a collection of modes
