...
if (nargin == 3) && isbinary
    ...
    % Case IIe: If Y is a symtensor
    if isa(X,'tensor') && isa(Y,'symtensor')
            Z = fun(full(Y),X);
        return;
    end
    
    % Case IIf: Either X or Y is not a tensor   
...
X = varargin;  %A cell array of inputted tensors
n = numel(X);
sz = size(X{1});

for i=2:n  %Verify that tensors are same size
    if ~isequal(size(X{i}),sz)
        error('Tensors are not same size');
    end
end

m = prod(sz);   %Number of elements in X.data
Y = zeros(m,n);  %An array of linearized X.data arrays. 
for j = 1:n
    Y(:,j) = X{j}.data(:);
end
data = zeros(m,1);
