...
%   Each entry of RNG must be in the set {1,...,ndims(K)}. The gradient
%   with respect to the Nth factor matrix, i.e., K.U{N}, is a matrix of
%   size I x R where I = size(K,N) and R = ncomponents(K).
%
...
%      'GIL': Gaussian Identity Link where L = (X-K).^2 [Default]
%      'PLL': Poisson Log Link where L = exp(full(K)) - X.*K
%      'PIL': Poisson Identiry Link where L = K - X.*log(full(K))
%      'BLL': Bernoulli Logit Link where L = log(1+K) - X.*K
%
...
%   and is a subset of {0,...,N}. The value zero indiates to regularlize
...
params.addParameter('Type', 'GIL', @(x) ismember(x, {'GIL','PLL','PIL','BLL'}));
